	...
	function graphResposta1(){
		try{	
               ...
               //Script SQL para solicitar as respostas da pergunta 1 do questionario
                $sql = DB::getConn()->prepare("SELECT DISTINCT resposta.descricao AS genero,COUNT(resposta.descricao) AS quantidade FROM `resposta` INNER jOIN `pergunta`ON resposta.Pergunta_id = pergunta.id_pergunta WHERE Pergunta_id =1 AND pergunta.Questionario_id=1 GROUP BY resposta.descricao");
                 $sql->execute();
